...
class LobbyUIManager:
    ...
    def update_positions(self, scale_factor, offset_x, offset_y, buttons):
        ...
        instruction_base_y = (self.screen_height - 140) * scale_factor + offset_y
        ...
        for i, btn in enumerate(buttons):
            if btn.label == "НАЧАТЬ ИГРУ":
                ...
                btn.center_y = (self.screen_height - 40) * scale_factor + offset_y
    ...
    def _draw_selected_info(self, selected_character, character_slots):
        if selected_character and self.selected_title_text:
            selected_slot = next((s for s in character_slots if s.character_id == selected_character), None)
            if selected_slot:
