...
class LobbySlot:
    ...
    def update_position(self, scale_factor, offset_x=0, offset_y=0, delta_time: float = 1/60):
        ...
        self.ha_text.position = (
            self.center_x, self.center_y + 40 * scale_factor)
        self.ha_text.font_size = int(36 * scale_factor)

        self.text_code.position = (
            self.center_x, self.center_y + 40 * scale_factor)
        self.text_code.font_size = int(14 * scale_factor)

        self.text_subcode.position = (
            self.center_x, self.center_y + 20 * scale_factor)
        self.text_subcode.font_size = int(12 * scale_factor)

        self.text_name.position = (
            self.center_x, self.center_y - 70 * scale_factor)
        self.text_name.font_size = int(20 * scale_factor)

        self.text_desc.position = (
            self.center_x, self.center_y - 100 * scale_factor)
        self.text_desc.font_size = int(14 * scale_factor)
        self.text_desc.width = 180 * scale_factor

        self.text_selected.position = (
            self.center_x, self.center_y - 130 * scale_factor)
        self.text_selected.font_size = int(16 * scale_factor)

        self.text_locked.position = (
            self.center_x, self.center_y + 90 * scale_factor)
    ...
    def draw(self):
        ...
        if self.sprite:
            ...
            circle_color = self.character_colors.get(
                self.character_id, (100, 150, 200))
